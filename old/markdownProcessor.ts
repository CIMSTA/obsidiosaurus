...
export default async function processMarkdown(
	...
	for await (const line of rl) {
		...
		processedLine = checkForAssets(
			processedLine,
			processedFileName,
			assetJson
		);
		processedLine = checkForLinks(processedLine);
		[processedLine, inAdmonition, inQuote, admonition] = convertAdmonition(
			processedLine,
			inAdmonition,
			inQuote,
			admonition
...
const convertAdmonition = (
	line: string,
	isInAdmonition: boolean,
	isInQuote: boolean,
	admonition: Admonition
): [string, boolean, boolean, Admonition] => {
...
function checkForAssets(
	...
	if (match && match.groups) {
		...
		let existingAsset = assetJson.find(
			(item) => item.fileName === fileName
		);
		let existingSize: Size;
		if (existingAsset) {
			existingSize = getOrCreateSize(
				existingAsset.sizes,
				size,
				processedFileName
			);
...
function processImage(
	...
	if (extensionFormatMap[fileExtension]) {
		...
		if (fileExtension === "excalidraw") {
			const darkPath = newPath.replace(
				".light.svg#light",
				".dark.svg#dark"
			);
			line += `\n![${fileName}](${darkPath})`;
